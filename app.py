...
if uploaded:
    ...
    df = prepare_df(df)
    if df is not None:
        # Build column options from the data actually present (1..12 typical for 96-well)
        present_cols = sorted(int(c) for c in pd.unique(df["Col"].dropna()) if 1 <= int(c) <= 24)
        default_samples = [c for c in range(1, 10) if c in present_cols]  # 1–9 if present
        default_qc = [c for c in [11, 12] if c in present_cols]          # 11,12 if present
